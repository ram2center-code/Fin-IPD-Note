...
const Navbar = ({ activeTab, setActiveTab, user, userRole, dueCount, pendingCasesCount, onLogout, isMuted, setIsMuted, onOpenNotifications }) => {
    ...
    <nav className="sticky top-0 z-[1000] bg-white/80 backdrop-blur-xl border-b border-slate-200 py-3">
      <div className="max-w-[1200px] mx-auto px-6 flex justify-between items-center">
        ...
        <div className="flex gap-1 bg-slate-100 p-1 rounded-xl border border-slate-200/50 scale-95 md:scale-100">
          <button 
            className={`flex items-center gap-2 px-3 md:px-5 py-2 rounded-lg font-bold text-xs md:text-sm transition-all duration-300 relative ${
              activeTab === 'dashboard' 
          ...
          >
            ...
            {typeof pendingCasesCount === 'number' && pendingCasesCount > 0 && (
              <span className="absolute -top-2.5 -right-2 bg-orange-500 text-white text-[9px] font-black px-1.5 py-0.5 rounded-md shadow-sm border border-white whitespace-nowrap">
                {pendingCasesCount} เคสค้าง
              </span>
            )}
          </button>
